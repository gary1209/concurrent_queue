...
#define QUEUE_NAME "LockBasedQueue"
...
void performEnqueue(LockBasedQueue& queue, int thread_id, bool lowContention) {
...
void performDequeue(LockBasedQueue& queue, int thread_id, bool lowContention) {
    for (int i = 0; i < OPERATIONS_PER_THREAD; ++i) {
        if (lowContention && !performOperationWithLowContentionProbability()) continue;
...
void performMixedOperations(LockBasedQueue& queue, int thread_id, bool lowContention) {
    for (int i = 0; i < OPERATIONS_PER_THREAD; ++i) {
        if (lowContention && !performOperationWithLowContentionProbability()) continue;
...
void runScenario(void (*operation)(LockBasedQueue&, int, bool), LockBasedQueue& queue, const char* scenarioName, bool lowContention) {
...
void performSPMC(LockBasedQueue& queue, bool lowContention, const char* scenarioName) {
    double start_time = omp_get_wtime();
...
void performSCMP(LockBasedQueue& queue, bool lowContention, const char* scenarioName) {
    double start_time = omp_get_wtime();
